---
title: Adding watermarks to a video
description: The most basic Pipeless example adding a watermark to a video
---
...
```bash copy
wget -O - https://github.com/pipeless-ai/pipeless/archive/main.tar.gz | tar -xz --strip=2 "pipeless-main/examples/watermark"
```
...
def hook(frame, context):
     ...
     text = "Hello pipeless!"
     font = ImageFont.truetype(font='/home/path/pipeless/examples/watermark/font.ttf', size=60)
...
```bash copy
pipeless add stream --input-uri "v4l2" --output-uri "screen" --frame-path "watermark"
```
